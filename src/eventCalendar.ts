...
const updateCalendar = () => {
    ...
    const days = [ //日を列挙
        ...Array(firstDayOfWeek).fill("").map(_ => {return {date: "", event: []}}), //前の月の部分を空白で埋める
        ...Array(lastDay.getDate()).fill("").map((_, idx) => {return { //日付の分だけ列挙する
            date: idx + 1,
    ...
    days.forEach(d => {
        ...
        <td>
            ...
            <div class="markRow">
                ${d.event.map(e =>
                    `<span class="mark" data-type="${e.type}"></span>`
                ).join("")}
            </div>
        </td>
        `) //htmlを作る
